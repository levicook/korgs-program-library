use {
    pinocchio_sol_vault_client::instructions::WithdrawV1Ix,
    solana_hash::Hash,
...
/// Builder for creating malicious `WithdrawV1` instructions.
#[derive(Debug, Clone)]
pub struct MaliciousWithdrawV1Ix {
    program_id: Pubkey,
    owner: AccountMeta,
    vault: AccountMeta,
...
impl MaliciousWithdrawV1Ix {
    ...
    pub fn from_valid(program_id: Pubkey, owner: Pubkey, amount: u64) -> Self {
        let valid = WithdrawV1Ix::new(program_id, owner, amount);
        let mut instruction_data = vec![3u8]; // WithdrawV1 discriminator
    ...
    pub fn with_invalid_data(mut self) -> Self {
        self.instruction_data = vec![3u8, 0u8, 1u8, 2u8]; // Only 4 bytes instead of 9
        self
    }
...
/// Builder for creating malicious `WithdrawV1` transactions.
#[derive(Debug)]
pub struct MaliciousWithdrawV1Tx {
    program_id: Pubkey,
    owner_kp: Keypair,
...
impl MaliciousWithdrawV1Tx {
    ...
    pub fn from_valid(
        ...
        let valid_ix = WithdrawV1Ix::new(program_id, owner_kp.pubkey(), amount)
            .build()
            .unwrap();
        Self {
            program_id,
    ...
    where
        F: FnOnce(MaliciousWithdrawV1Ix) -> MaliciousWithdrawV1Ix,
    {
        ...
        let malicious_ix =
            MaliciousWithdrawV1Ix::from_valid(self.program_id, self.owner_kp.pubkey(), amount);
