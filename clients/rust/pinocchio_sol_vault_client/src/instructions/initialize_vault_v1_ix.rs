...
pub enum InitializeVaultV1IxError {
...
pub struct InitializeVaultV1Ix {
...
impl InitializeVaultV1Ix {
    ...
    /// # Errors
    ///
    /// Returns [`InitializeVaultV1IxError`] if validation fails.
    pub fn validate(&self) -> Result<(), InitializeVaultV1IxError> {
        if !self.payer.is_signer {
            return Err(InitializeVaultV1IxError::PayerMustBeSigner);
        }

        if !self.payer.is_writable {
            return Err(InitializeVaultV1IxError::PayerMustBeWriteable);
        }

        if !self.vault.is_writable {
            return Err(InitializeVaultV1IxError::VaultMustBeWriteable);
        }
        ...
        if observed_vault != expected_vault {
            return Err(InitializeVaultV1IxError::VaultAddressMismatch {
        ...
        if observed_system_program != expected_system_program {
            return Err(InitializeVaultV1IxError::SystemProgramAddressMismatch {
    ...
    ///
    /// Returns [`InitializeVaultV1IxError`] if validation fails.
    pub fn build(self) -> Result<Instruction, InitializeVaultV1IxError> {
        self.validate()?;

        let instruction_data = [u8::from(InstructionDiscriminator::InitializeVaultV1)];
