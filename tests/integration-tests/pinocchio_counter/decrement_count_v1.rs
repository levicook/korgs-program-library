use {
    crate::{
        ...
        pinocchio_counter::{
            malicious_builders::{MaliciousDecrementCountV1Ix, MaliciousDecrementCountV1Tx},
    ...
    pinocchio_counter_client::{
        ...
        transactions::{
            DeactivateCounterV1SimpleTx, DecrementCountV1SimpleTx, IncrementCountV1SimpleTx,
            InitializeCounterV1SimpleTx,
        },
    },
...
fn succeeds() -> TestResult {
    ...
    let decrement_tx =
        DecrementCountV1SimpleTx::try_new(ctx.program_id(), owner_kp, ctx.latest_blockhash())?;
    ...
    // Decrementing from 0 saturates at 0
    assert_eq!(counter_after.count, 0);
...
fn succeeds_multiple_times() -> TestResult {
    ...
    // First increment to 5, then decrement back down
    for _ in 0..5 {
    ...
    // Now decrement multiple times
    for expected_count in (0..=4).rev() {
        let decrement_tx = DecrementCountV1SimpleTx::try_new(
            ctx.program_id(),
            owner_kp.insecure_clone(),
            ctx.latest_blockhash(),
        )?;
        ...
        assert_eq!(
            ...
            "Expected count {} after decrement, got {}",
            expected_count, counter.count
        );
    }
...
fn fails_when_owner_not_signer() -> TestResult {
    ...
    demand_logs_contain("failed: custom program error: 0x402", &tx_result);
...
fn fails_when_counter_not_writable() -> TestResult {
    ...
    let tx_result = ctx.send_transaction(malicious_tx);
    demand_tx_failure(&tx_result);
    demand_logs_contain("failed: custom program error: 0x404", &tx_result);
...
fn fails_when_counter_address_mismatch() -> TestResult {
    ...
    let malicious_tx =
        MaliciousDecrementCountV1Tx::from_valid(ctx.program_id(), owner_kp, ctx.latest_blockhash())
            .with_malicious_instruction(super::malicious_builders::decrement_count_v1::MaliciousDecrementCountV1Ix::with_random_counter_address)
            .build();

    let tx_result = ctx.send_transaction(malicious_tx);
    demand_tx_failure(&tx_result);
    demand_logs_contain("failed: custom program error: 0x405", &tx_result);
...
fn fails_when_not_enough_accounts() -> TestResult {
    ...
    let malicious_ix = MaliciousDecrementCountV1Ix::from_valid(ctx.program_id(), owner_pk);
    ...
    let tx_result = ctx.send_transaction(malicious_tx);
    demand_tx_failure(&tx_result);
    demand_logs_contain("failed: custom program error: 0x401", &tx_result);
...
fn fails_when_counter_not_initialized() -> TestResult {
    ...
    // Don't initialize counter - try to decrement non-existent counter
